...
public class QuakeEntry implements Comparable<QuakeEntry> {
    ...
    public int compareTo(QuakeEntry loc) {
        ...
        return 0;
         */
        // Sort by magnitude first, then by depth
        if (this.getMagnitude() > loc.getMagnitude()) {
            return 1;
        } else if (this.getMagnitude() < loc.getMagnitude()) {
            return -1;
        } else {
            if (this.getDepth() > loc.getDepth()) {
                return 1;
            } else if (this.getDepth() < loc.getDepth()) {
                return -1;
            } else {
                return 0;
            }
